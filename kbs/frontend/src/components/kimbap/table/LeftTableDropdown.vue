    ...
    <div class="grid grid-cols-2 gap-x-8 gap-y-4">
      <div v-for="(field, index) in fields" :key="field.field" class="flex items-center space-x-2">
        ...
        <div class="flex-1 relative">
          ...
          <template v-else-if="field.type === 'dropdown' || field.type === 'select'">
              <select
                  :value="data[field.field]"
                  @change="updateField(field.field, $event.target.value)"
                  :disabled="field.disabled"
                  class="border rounded p-2 w-full h-10 bg-white"
              >
